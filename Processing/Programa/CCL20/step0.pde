void step0(){
  ...
  boolean offLinea1 = false;
  boolean offLinea7 = false;
...
//Lanzamos la función vector con interacción Lerp
  PVector current6 = new PVector (lerp (end[1].x, end[6].x, m[8]), lerp (end[1].y, end[6].y, m[8]));
...
//Cambiamos el valor de la posición del vertice
  Xver1 = current9.x; 
  Yver1 = current9.y;
...
//------------------Movemos vertice 2- posición 1------------------------

//Definimos el final del vector
  end[10] = new PVector (707, 637);
  
//Lanzamos la función vector con interacción Lerp
  PVector current10 = new PVector (lerp (end[1].x, end[10].x, m[9]), lerp (end[1].y, end[10].y, m[9]));
...
//Definimos el final del vector
  start[11] = new PVector (965, 474);
  end[11] = new PVector (965, 439);
...
//------------------Movemos vertice 1- posición 2------------------------
//Esperamos 4 segundos desde el fin del anterior movimiento
if (m[12] >= 1){
    
  if(tempTime2 == false){  //Condición para hacer en este momento la carga del tiempo
    time2 = millis();   //Cargamos el tiempo actual
    tempTime2 = true;
  }
  
  if(millis() - time2 >= wait2){
    ver1temp2 = true;       
      time2 = millis();  //Reiniciamos el temporizador
  } 
}
...
//Lanzamos la función vector con interacción Lerp
  PVector current13 = new PVector (lerp ( end[9].x, end[3].x, m[13]), lerp ( end[9].y,end[3].y, m[13]));
...
//------------------Movemos vertice 2- posición 2------------------------

//Definimos el final del vector
  end[12] = new PVector (707, 637);
  
//Lanzamos la función vector con interacción Lerp
  PVector current10 = new PVector (lerp (end[10].x,end[8].x, m[13]), lerp (end[10].y,end[8].y, m[13]));
